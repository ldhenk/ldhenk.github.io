function GLController(gl, cwidth, cheight){
	...
	function projMat(fov,zNear,flipHoriz,flipVert){
		let rwidth = width;
		let rheight = height;
		if(fbBound !== null){
		...
		*/
		let k5 = -1;
		let k2 = 1/(Math.tan(fov/2));
		let k1 = rheight/rwidth * k2;
		//let k1 = 1/(Math.tan(fov/2));
	...
	// Project y and z but preserve x as isometric
	this.jankyProjMat = function(fov,zNear){
		let rwidth = width;
		let rheight = height;
		if(fbBound !== null){
			rwidth = cfbo[fbBound].getWidth();
		...
		*/
		...
		let k6 = 1/k1; //TODO play with this value
	...
	this.projMat = function (fov,zNear){
		return projMat(fov,zNear,false,false);
	};

	this.mirrorProjMat = function (fov,zNear){
		return projMat(fov,zNear,true,false);

	};

	this.vertMirrorProjMat = function (fov,zNear){
		return projMat(fov,zNear,false,true);

	};

	this.doubMirrorProjMat = function (fov,zNear){
		return projMat(fov,zNear,true,true);
	...
	*/
	...
	this.modelMatNonUniformScaling = function(x, y, z, yaw, pitch, roll, sx, sy, sz){
		return mh.modelMatNonUniformScaling(x, y, z, yaw, pitch, roll, sx, sy, sz);
	};
